...
struct node* insert(struct node* node, float value, int freq)
{
	...
	if (value < node->value)
		node->left = insert(node->left, value, freq);
	else
		node->right = insert(node->right, value, freq);
...
{   
    ...
    if (n->right != NULL)
    {
        struct node* min = findMinimum(n->right);
        return min;
    }
...
struct node* findPredecesor(struct node* root, struct node* n)
{
...
struct node* findMaxforN(struct node* root, float N)
{
    // Base cases
    if (root == NULL)
    {
    ...
    // subtree
        ...
        struct node* k = findMaxforN(root->right, N);
        if (k == NULL)
            return root;
        else
            return k;
...
struct node* find(struct node *root, float key) {
