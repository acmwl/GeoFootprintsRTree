...
public:
  ...
  int MySearchBatch(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], Region queryRegions[], 
                    int queryRegionsSize, float overlapCounters[]) const;

  // MySearchBatch, returns all indexes of queries that overlap with a leaf node
  vector<int> mySearchBatch(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS]) const;
...
protected:
  ...
  bool Overlap(Region* a_rectA, Rect* a_rectB) const;
  void ReInsert(Node* a_node, ListNode** a_listNode);
  bool Search(Node* a_node, Rect* a_rect, int& a_foundCount, std::function<bool (const DATATYPE&)> callback) const;
  bool MySearchIterative(Node* a_node, Rect* a_rect, int& a_foundCount, float overlapCounters[]) const;
  bool MySearchBatch(Node* a_node, Rect* a_rect,Rect* a_parentRect, int& a_foundCount, Region queryRegions[], 
                    int queryRegionsSize, float overlapCounters[]) const;
  bool MySearchBatch(Node* a_node, Rect* a_rect, vector<int>& indexes) const;
  float calculateOverlap(Rect* a_rectA, Rect* a_rectB) const;
  float calculateOverlap(Region* a_rectA, Rect* a_rectB) const;
  ...
  float mySweep(int queryRegionsSize, Region *queryRegions, int userRegionsSize, Branch *userRegions,float overlapCounters[]) const;
  void mySweepLoop(Branch rec, Region *pivot, int pivotStart,int pivotSize, float overlapCounters[]) const;
  void mySweepLoop(Region rec, Branch *pivot, int pivotStart,int pivotSize, float overlapCounters[]) const;
  //bool compareBranches(Branch branchA, Branch branchB) const;
...
RTREE_TEMPLATE
int RTREE_QUAL::MySearchBatch(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], Region queryRegions[], 
                 int queryRegionsSize, float overlapCounters[]) const{
  ...
  int foundCount = 0;       //doesnt matter
  MySearchBatch(m_root, &rect, &rect, foundCount, queryRegions, queryRegionsSize, overlapCounters);
...
vector<int> RTREE_QUAL::mySearchBatch(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS]) const{
  ...
  // NOTE: May want to return search result another way, perhaps returning the number of found elements here.
  vector<int> indexes;
  MySearchBatch(m_root, &rect, indexes);
  return indexes;
}
...
{
  ASSERT(a_rectA && a_rectB);
  //cout<<"Comparing recta:"<<a_rectA->m_min[0]<<","<<a_rectA->m_min[1]<<","<<a_rectA->m_max[0]<<","<<a_rectA->m_max[1]<<endl;
  //cout<<"With rectb:"<<a_rectB->m_min[0]<<","<<a_rectB->m_min[1]<<","<<a_rectB->m_max[0]<<","<<a_rectB->m_max[1]<<endl;
  for(int index=0; index < NUMDIMS; ++index)
  {
    if (a_rectA->m_min[index] >= a_rectB->m_max[index] ||
        a_rectB->m_min[index] >= a_rectA->m_max[index])
    {
      //cout<<"resultIsFalse"<<endl;
      return false;
    }
  }
  //cout<<"ResultIsTrue"<<endl;
  return true;
}

// Decide whether two rectangles overlap.
RTREE_TEMPLATE
bool RTREE_QUAL::Overlap(Region* a_rectA, Rect* a_rectB) const
{
  ASSERT(a_rectA && a_rectB);
  //cout<<"Comparing regiona:"<<a_rectA->xStart<<","<<a_rectA->yStart<<","<<a_rectA->xEnd<<","<<a_rectA->yEnd<<endl;
  //cout<<"With rectb:"<<a_rectB->m_min[0]<<","<<a_rectB->m_min[1]<<","<<a_rectB->m_max[0]<<","<<a_rectB->m_max[1]<<endl;

  if (a_rectA->xStart >= a_rectB->m_max[0] ||
        a_rectB->m_min[0] >= a_rectA->xEnd)
    {
      //cout<<"resultIsFalse"<<endl;
      return false;
    }

  if (a_rectA->yStart >= a_rectB->m_max[1] ||
        a_rectB->m_min[1] >= a_rectA->yEnd)
    {
      //cout<<"resultIsFalse"<<endl;
      return false;
    }
...
/*
RTREE_TEMPLATE
bool RTREE_QUAL::compareBranches(Branch branchA, Branch branchB) const{
  return (branchA.m_rect.m_min[0]<branchB.m_rect.m_min[0]);
}*/
...
// Calculate the overlapping area between two overlapping rectangles
RTREE_TEMPLATE
...
// Calculate the overlapping area between two overlapping rectangles
RTREE_TEMPLATE
float RTREE_QUAL::calculateOverlap(Region* a_rectA, Rect* a_rectB) const
{
  float result = 1;

  result *= Min(a_rectA->xEnd,a_rectB->m_max[0])-
            Max(a_rectA->xStart,a_rectB->m_min[0]);
...
// Iterative Search with overlapping area calculation
// Updates overlapCounters, an array with overlapping areas between query and each user
RTREE_TEMPLATE
bool RTREE_QUAL::MySearchIterative(Node* a_node, Rect* a_rect, int& a_foundCount, float overlapCounters[]) const
{
  ASSERT(a_node);
  ASSERT(a_node->m_level >= 0);
  ASSERT(a_rect);
  ...
  if(a_node->IsInternalNode())
  {
    ...
    for(int index=0; index < a_node->m_count; ++index)
    {
      if(Overlap(a_rect, &a_node->m_branch[index].m_rect))
      {
        MySearchIterative(a_node->m_branch[index].m_child, a_rect, a_foundCount, overlapCounters);
      }
    }
  }
  else
...
RTREE_TEMPLATE
void RTREE_QUAL::mySweepLoop(Region rec, Branch *pivot, int pivotStart,int pivotSize, float overlapCounters[]) const
{
	int counter = pivotStart;
	while(counter < pivotSize && rec.xEnd >= pivot[counter].m_rect.m_min[0])
	{
		if(rec.yStart > pivot[counter].m_rect.m_max[1] || rec.yEnd < pivot[counter].m_rect.m_min[1])
		{
			counter++;
			continue;
		}
		float minX = max(rec.xStart,pivot[counter].m_rect.m_min[0]);
...
RTREE_TEMPLATE
void RTREE_QUAL::mySweepLoop(Branch rec, Region *pivot, int pivotStart,int pivotSize, float overlapCounters[]) const
{
	int counter = pivotStart;
	while(counter < pivotSize && rec.m_rect.m_max[0] >= pivot[counter].xStart)
	{
		if(rec.m_rect.m_min[1] > pivot[counter].yEnd || rec.m_rect.m_max[1] < pivot[counter].yStart)
		{
			counter++;
			continue;
		}
		float minX = max(rec.m_rect.m_min[0],pivot[counter].xStart);
...
RTREE_TEMPLATE
float RTREE_QUAL::mySweep(int queryRegionsSize, Region *queryRegions, int userRegionsSize, Branch *userRegions, float overlapCounters[]) const
{
	int i=0,j=0;
	int currentR = 0, currentS = 0;
	float area=0;

  while(currentR < queryRegionsSize && currentS < userRegionsSize)
  {
      // printf("i = %d, j = %d\n",currentR,currentS);
      // printf("Before interval %d %d\n",i,userRegionsSize/4);
      // printf("Before interval %d %d\n",j,queryRegionsSize/4);
      if(queryRegions[currentR].xStart < userRegions[currentS].m_rect.m_min[0])
      {
          // cout << "1 " << currentS << " " << currentR<< endl;
          mySweepLoop(queryRegions[currentR], userRegions, currentS, userRegionsSize, overlapCounters);
          currentR++;
      }
      else
      {
          // cout << "2" << endl;
          mySweepLoop(userRegions[currentS], queryRegions, currentR, queryRegionsSize, overlapCounters);
          currentS++;
      }
  }
	return area;
...
RTREE_TEMPLATE
bool RTREE_QUAL::MySearchBatch(Node* a_node, Rect* a_rect, Rect* a_parentRect,  int& a_foundCount, 
                              Region queryRegions[], int queryRegionsSize, float overlapCounters[]) const{
  //like iterative, search for rect (which is the mbr of query)
  //when hitting leaf, search for overlap between each query and each entry while updating overlapCounters
  // the "search for overlap" part could have optimizations maybe
  // for plane sweep, we need to sort both query regions and leaf regions
  ASSERT(a_node);
  ASSERT(a_node->m_level >= 0);
  ASSERT(a_rect);
  ...
  if(a_node->IsInternalNode())
  {
    ...
    for(int index=0; index < a_node->m_count; ++index)
    {
      if(Overlap(a_rect, &a_node->m_branch[index].m_rect))
      {
        MySearchBatch(a_node->m_branch[index].m_child, a_rect, &a_node->m_branch[index].m_rect, a_foundCount, 
                queryRegions, queryRegionsSize, overlapCounters);
      }
    }
  }
  else
  {
    ...
    int querySkimmed[queryRegionsSize] = {};
    for(int qindex=0; qindex < queryRegionsSize; ++qindex) {
      if(Overlap(&queryRegions[qindex], a_parentRect)){
        querySkimmed[querySkimmedIndex]=qindex;
        ++querySkimmedIndex;
      }
      //if query is sorted, break
    }
    ...
    for(int index=0; index < a_node->m_count; ++index) {
      ...
      for(int qindex=0; qindex < querySkimmedIndex; ++qindex) {
        if(Overlap(&queryRegions[querySkimmed[qindex]], &a_node->m_branch[index].m_rect)) {
    ...
    for(int index=0; index < a_node->m_count; ++index) {
      ...
      for(int qindex=0; qindex < queryRegionsSize; ++qindex) {
        if(Overlap(&queryRegions[qindex], &a_node->m_branch[index].m_rect)) {
    ...
    if(!a_node->m_sorted){
      sort(a_node->m_branch,a_node->m_branch+a_node->m_count,
        [](const Branch & first, const Branch & second) { 
          return (first.m_rect.m_min[0]<second.m_rect.m_min[0]) ;
        });
      a_node->m_sorted=true;
    }
...
bool RTREE_QUAL::MySearchBatch(Node* a_node, Rect* a_rect, vector<int>& indexes) const{
  ...
  if(a_node->IsInternalNode()) {
    ...
    for(int index=0; index < a_node->m_count; ++index) {
      if(Overlap(a_rect, &a_node->m_branch[index].m_rect)) {
        MySearchBatch(a_node->m_branch[index].m_child, a_rect, indexes);
      }
    }
  }
  else
